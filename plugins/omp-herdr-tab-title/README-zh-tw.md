...
omp extension:把目前的 herdr 分頁改名為 omp session 標題,最多 15 顯示欄;超過時截到 15 欄並加上 `…`(共 16 欄)。herdr 分頁預設只有編號,難以辨識;這個 plugin 把 omp 本來就會產生的 session 名稱同步到分頁上。
...
- 標籤做寬度感知截斷:CJK 字算 2 欄;標題超過 15 欄時截到 15 欄並加上 `…`(共 16 欄)。
