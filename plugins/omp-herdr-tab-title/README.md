...
An omp extension that renames the current herdr tab to the omp session title, shortened to at most 12 display columns. herdr tabs default to bare numbers, which are hard to tell apart; this mirrors the session name omp already generates onto the tab.
