...
2. Inside a herdr tab, run `omp` in a scratch dir, send one prompt, and wait for the turn to end. The auto title appears within ~15 s; check with `herdr tab get $HERDR_TAB_ID` that `label` equals the ≤12-column prefix of the session title.
