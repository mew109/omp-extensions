...
將內建狀態列的 `path`、`git`、`pr` 三段合併成一行(另加 Open-Meteo 天氣段與 Yahoo 股市段)。渲染面可設定:`status`(預設)顯示在內建狀態列下方,`widget` 顯示在編輯器下方。前三段**改寫自 omp 內建渲染器**(path = mpi、git = cpi、pr = dpi;格式與縮排策略於 2026-08 對照已安裝的 @oh-my-pi/pi-coding-agent bundle 驗證);weather 與 stock 為本擴充自製。
...
非 git repo、無 PR、無天氣、無股市資料時該段不顯示(path 恆有);各段皆無時整行清空。

## 寬度

整行必須塞進一列:超寬時從最右段(`SEGMENTS` 尾端)開始縮,先縮到該段 min,再低於 min(下限 1)繼續往右縮,直到放得下。各段 cell 預算(max / min 定義在各 segment 檔):

寬度預算依渲染面而異:`widget` 面下,omp 把每行 `setWidget` 內容包在 `Text(line, 1, 0)` 裡,內容區為 `columns − 2`(`tui.tight` 會拿掉 padding,該設定下只是多出 2 cells 餘裕);`status` 面下整行以完整 `columns` 計算預算,超出由 host 以刪節號截斷。寬度以 grapheme cluster 為單位、用 `Bun.stringWidth` 計算——與 renderer 換行的引擎相同:emoji + VS16 = 2 cells、`⛅`/`➖` = 2、tab = 3。此修正解決低估寬度導致窄終端上整行換到第二列的問題。
...
YAML(JSON 亦合法),schema 模仿 omp 內建 `statusLine`;以下值即內建預設:

    segments: [path, git, pr, weather, stock]   # 順序 = 顯示順序(最尾端先縮)
    surface: status                             # 渲染面:status(內建狀態列下方)/ widget(編輯器下方)
    segmentOptions:                             # 每段寬度上下限,cells
      path:    { max: 40, min: 24 }
      git:     { max: 36, min: 20 }
      pr:      { max: 30, min: 10 }
      weather: { max: 39, min: 20 }
      stock:   { max: 38, min: 20 }
    weather: { location: Tokyo, lang: zh }      # 地名(自動轉座標)/ 標籤語言 zh 或 en
    stock:   { symbol: "^TWII", name: TAIEX }   # Yahoo 代號 / 顯示名稱
...
- `surface`:渲染面,`status` 或 `widget`(預設 `status`);比對前先 trim、不分大小寫,空白或未知值回落預設。環境變數 `ANOTHER_SURFACE` 蓋過之。
- 渲染面行為——`status`:每個 extension key 一行,依 key 字母序排列(穩定,不隨更新順序浮動),顯示在內建狀態列下方;host 會剝除 ANSI/OSC escape、壓縮連續空白,並在終端寬度處以刪節號截斷;該行於下次 host 重繪時更新,背景輪詢的資料可能晚一個按鍵才出現。`widget`:顯示在編輯器與狀態列之間(`belowEditor`),立即重繪;有多個 widget extension 時,順序跟隨更新時間。
- `segmentOptions.<id>.max` / `min`:每段寬度上下限(cells),五個 segment id(`path` / `git` / `pr` / `weather` / `stock`)皆可設定。正整數(≥1),合併後需 min ≤ max;單一欄位無效則忽略該欄位用內建值;min > max 則該段 max / min 全用內建值。無對應 segment 的 key 忽略。
...
- 環境變數逐 key 蓋過設定檔,設定檔再蓋過內建(**env > YAML > 內建**):`ANOTHER_WEATHER_LOCATION`、`ANOTHER_WEATHER_LANG`、`ANOTHER_STOCK_SYMBOL`、`ANOTHER_STOCK_NAME`、`ANOTHER_SURFACE`。空白字串視同未設;值會 trim。
